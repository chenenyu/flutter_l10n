...
LocalizationsGenerator generateLocalizations({
  ...
  final String templateArbFileName =
      options.templateArbFile?.toFilePath() ?? 'l10n_en.arb';
  final String outputFileString =
      options.outputLocalizationsFile?.toFilePath() ?? 'l10n.dart';
  ...
  try {
    generator = LocalizationsGenerator(
      ...
      classNameString: options.outputClass ?? 'S',
...
[${_inputFileNames[locale]}:${message.resourceId}] ICU Syntax Warning: The plural part specified below is overridden by a later plural part.
    ...
    ${Parser.indentForError(pluralPart.positionInMessage)}''');
      ...
      return methodTemplate
          ...
          .replaceAll('@(none)\n', '');
    } on L10nParserException catch (e) {
      error(e.toString());
      hadErrors = true;
      return '';
    }
  ...
  List<String> writeOutputFiles({bool isFromYaml = false}) {
    ...
    if (messagesFile != null) {
      _generateUntranslatedMessagesFile(messagesFile);
    } else if (_unimplementedMessages.isNotEmpty) {
      _unimplementedMessages
          .forEach((LocaleInfo locale, List<String> messages) {
        info('"$locale": ${messages.length} untranslated message(s).');
      });
      if (isFromYaml) {
        info('To see a detailed report, use the untranslated-messages-file \n'
            'option in the flutter_l10n.yaml file:\n'
            'untranslated-messages-file: desiredFileName.txt\n'
            '<other option>: <other selection> \n\n');
      } else {
        info('To see a detailed report, use the --untranslated-messages-file \n'
            'option in the flutter_l10n tool:\n'
            'flutter_l10n generate --untranslated-messages-file=desiredFileName.txt\n'
            '<other options> \n\n');
      }
  ...
  void _generateUntranslatedMessagesFile(File untranslatedMessagesFile) {
